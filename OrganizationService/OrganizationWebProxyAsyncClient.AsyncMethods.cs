...
{
    public partial class OrganizationWebProxyAsyncClient
    {
        #region Create
        ...
        internal IAsyncResult BeginUpdate(Entity entity, AsyncCallback callback, object asyncState, CancellationToken cancellationToken)
            => AsyncCallOperation<int>.Begin("Update", callback, asyncState, cancellationToken,
                ...
                tryBegin: TryRun,
                tryEnd: TryRun,
                beginOperation: (_callback, _state) => base.Channel.BeginUpdate(entity, _callback, _state),
                endOperation: (_asynResult) => { base.Channel.EndUpdate(_asynResult); return 0; },
                cancelOperation: (_asyncResult) => base.Abort());
        ...
        internal IAsyncResult BeginAssociate(string entityName, Guid entityId, Relationship relationship, EntityReferenceCollection relatedEntities, AsyncCallback callback, object asyncState, CancellationToken cancellationToken)
            => AsyncCallOperation<int>.Begin("Associate", callback, asyncState, cancellationToken,
                ...
                tryBegin: TryRun,
                tryEnd: TryRun,
                beginOperation: (_callback, _state) => base.Channel.BeginAssociate(entityName, entityId, relationship, relatedEntities, _callback, _state),
                endOperation: (_asynResult) => { base.Channel.EndAssociate(_asynResult); return 0; },
                cancelOperation: (_asyncResult) => base.Abort());
